...
However, we repeat the experiment $10^6$ times and the correct init state lies in $L_{z_0z_1}$ for only 971842 times.
So it is safe for us to claim that the actual $p_1$ is $0.9718$: Zhang's evaluation in \cite{AC:Zhang19} is far too optimistic.
...
All these experiments reveal that the actual $p_1$ is 0.9718 rather than 0.9835: Zhang's evaluation in \cite{AC:Zhang19} is wrong.
...
\subsection{Zhang's Wrong Evaluation to the Complexities}
...
\cite{AC:Zhang19} only contains the evaluation to the process from Level 1 to 2. 
